...
TODO:
1. Organize the code
2. Remove the varification block, make the execution of varification a combination of action and varification. And will only be triggered action that are not just movements but some input.
3. Create dedicated functions for excel manupulation, includes the following: Jump to Cell, input data to cell, delete cell data, copy cell data, paste cell data
4. Create smaller flows for tool selection and step execution
5. refine the action plan generation prompts for better understanding of what an executable action plan is
6. create 2-3 common test cases
7. one of test cases should be creating a pivot table (few shot pivot table creation for higher accuracy)
8. Add switching between apps in case of switch between taking user input and so on.
8. finally package with a simple QT UI for user to input the task and get the output.
[^ Milestone 1]
'''
import os
import requests
...
response = step_creation_stage_1()
# [[IMP]] step_list
