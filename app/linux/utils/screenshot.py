...
from xpybutil import conn, root, ewmh, icccm, util
...
def capture_screenshot(
    ...
    def capture_desktop(save_path: Optional[str] = None):
        ...
        raw_image = root.get_image(0, 0, root.get_geometry().width, root.get_geometry().height, X.ZPixmap, 0xffffffff)
        image = Image.frombytes("RGB", (root.get_geometry().width, root.get_geometry().height), raw_image.data, "raw", "BGRX")
        if save_path:
            image.save(save_path)
        return image

    def capture_app_window(app_name: str, save_path: Optional[str] = None):
        windows = ewmh.get_client_list().reply()
        for win in windows:
            name = ewmh.get_wm_name(win).reply()
            if app_name in name:
                geom = win.get_geometry().reply()
                raw_image = root.get_image(geom.x, geom.y, geom.width, geom.height, X.ZPixmap, 0xffffffff)
                image = Image.frombytes("RGB", (geom.width, geom.height), raw_image.data, "raw", "BGRX")
                if save_path:
                    image.save(save_path)
                return image
    ...
    if screenshot_type == "desktop":
        ...
        if sub_control_titles:
            sub_controls = [win for win in ewmh.get_client_list().reply() if any(title in ewmh.get_wm_name(win).reply() for title in sub_control_titles)]
        else:
            sub_controls = []

        if output_format == "rectangle":
            for sub_control in sub_controls:
                geom = sub_control.get_geometry().reply()
                adjusted_rect = (geom.x, geom.y, geom.x + geom.width, geom.y + geom.height)
            ...
            for label_text, sub_control in annotation_dict.items():
                geom = sub_control.get_geometry().reply()
                adjusted_coordinate = (geom.x, geom.y)
...
if __name__ == "__main__":
    ...
    try:
        ...
        capture_screenshot(screenshot_type="app_window", app_name="gedit", save_path="app_window_screenshot.png")

        # Capture app window screenshot with rectangle
        capture_screenshot(screenshot_type="app_window", app_name="gedit", sub_control_titles=["Document"], output_format="rectangle", save_path="app_window_screenshot_with_rectangle.png")

        # Capture app window screenshot with annotations
        capture_screenshot(screenshot_type="app_window", app_name="gedit", sub_control_titles=["Document"], output_format="annotation", save_path="app_window_screenshot_with_annotations.png")
        ...
        base64_string = capture_screenshot(screenshot_type="app_window", app_name="gedit", output_format="base64")
