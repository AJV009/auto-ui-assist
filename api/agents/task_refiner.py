...
def task_refiner_agent(userid, sessionid, task, os):
    """
    Correct the task using the Anthropics API.
    """

    prompt_templates = prompt_loader("task_refiner")
    system_prompt = prompt_templates['system_prompt'].format(os=os)
    message_array = prompt_templates['fewshot_messages']
    message_array[-1]['content'] = message_array[-1]['content'].format(task=task)
      
    # dump to a json file for debugging
    with open("temp/task_refiner_agent.json", "w") as f:
...
def task_refiner_stage_2_agent(userid, sessionid, task, os):
    """
    Correct the task using the Anthropics API.
    """

    prompt_templates = prompt_loader("task_refiner_stage_2")
    system_prompt = prompt_templates['system_prompt'].format(os=os)
    message_array = prompt_templates['fewshot_messages']
    message_array[-1]['content'] = message_array[-1]['content'].format(task=task)
      
    # dump to a json file for debugging
    with open("temp/task_refiner_stage_2_agent.json", "w") as f:
      json.dump(message_array, f, indent=2)
    ...
    store_agent_conversation_array(
      userid=userid,
      sessionid=sessionid,
      agent_name="task_refiner_agent_stage_2",
      conversation_array=message_array
    )
